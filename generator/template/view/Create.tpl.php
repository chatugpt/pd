...
<form id="actionForm" name="actionForm" action="<?php echo zee::url('.$table.', "create_submit");?>" method="post">
<tbody>
		<tr>
			<td>
<!-- Form block start -->
<?php
$out'.$moduleName.'CreateValue = $inData["'.$moduleName.'CreateValue"];
?>

<table class="menutab" cellspacing="0" cellpadding="0" width="100%" border="0">
	<tbody>
		<tr>
			<td class="menumaintopon"><?php Language::show("'.strtoupper($table).'.CREATE.LABEL")?></td>
		</tr>
		<tr>
			<td>
...
<input type="text" class="textinput" name="'.$table.'_'.$field->name.'" value="<?php echo $out'.$moduleName.'CreateValue->'.$field->name.'?>" />
